            ...
            if (message.action === 'fileTransferRequest' && message.target === peerId) {
                ...
                if (accept) {
                    ...
                    peerConnection.onicecandidate = (event) => {
                        if (event.candidate) {
                            signalingSocket.send(JSON.stringify({ candidate: event.candidate, target: message.sender }));
                        }
                    };
                } else {
...
async function createOffer() {
    // if (!recipient) {
    //     console.error('Recipient is undefined. Cannot create offer.');
    //     return;
    // }
    const recipient = document.getElementById('recipientSelect').value;
    if (!recipient) {
        console.error('No recipient selected. Cannot create offer.');
        alert('Please select a recipient before sending the file.');
        return;
    }

    console.log('Creating WebRTC offer for recipient:', recipient);
    peerConnection = new RTCPeerConnection();
    ...
    peerConnection.onicecandidate = (event) => {
        if (event.candidate) {
            ...
            signalingSocket.send(JSON.stringify({ candidate: event.candidate, target: recipient }));
        }
    };
...
async function handleOffer(offer, sender) {
    ...
    if (!peerConnection || peerConnection.signalingState === 'closed') {
        console.log('Creating new RTCPeerConnection for offer handling.');
        peerConnection = new RTCPeerConnection();
        ...
        peerConnection.onicecandidate = (event) => {
            if (event.candidate) {
                console.log('Sending ICE candidate to sender:', sender);
                signalingSocket.send(JSON.stringify({ candidate: event.candidate, target: sender }));
            }
        };
    }
    ...
    try {
        ...
        console.log('Sending answer back to sender:', sender);
        signalingSocket.send(JSON.stringify({ answer, target: sender }));
    } catch (error) {
